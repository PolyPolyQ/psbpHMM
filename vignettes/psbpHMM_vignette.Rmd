...
|fitDPMM | fit Dirichlet process mixture model jointly to multiple time series |
|fixedK_clustering | fit a mixture of multivariate normal distributions to the data based on pre-specified clusters |
...
We can plot the data to see what it looks like. We plot all three exposures for one time series. 

```{r, cache = TRUE}
par(mfrow = c(1,3))
plot(1:288, dat$y[[1]][,1], col = dat$z.true[[1]], pch = 19, ylab = "exposure 1", xlab = "time", type = "l")
plot(1:288, dat$y[[1]][,2], col = dat$z.true[[1]], pch = 19, ylab = "exposure 2", xlab = "time", type = "l")
plot(1:288, dat$y[[1]][,3], col = dat$z.true[[1]], pch = 19, ylab = "exposure 3", xlab = "time", type = "l")
```
...
First, is a model without any covariates (1) in the transition distribution; that is, the probability of transitioning from state j to state k is: 
...
Second is a model with covariate-dependent transitions (2). The covariate-dependent transition distribution is 
...
Third is a model with subject-specific effects of the covariates in the transition distribution to account for repeated sampling days for the same individual (3). Here, the transition distribution is 
...
To begin, `niter` is the total number of MCMC iterations, `nburn` is the number of burn-in iterations, `y` is the list of data for fitting and may include missing observations, `rmlist` is a vector of integer labels for the repeated sampling days, `ycomplete` is optional and is the list of complete data, `X` is a list of time-varying covariates for each subject, and `priors` is the list of prior hyperparameters. The prior options for this model include the following: 
...
|mu.betaS | $\boldsymbol{\mu}_\gamma$ | $\mathbf{0}$ | mean parameter for Normal prior on $\boldsymbol{\gamma}_{ik}$ | 
|Sigma.betaS| $\boldsymbol{\Sigma}_\gamma$ | $\mathbf{I}$ | variance parameter for Normal prior on $\boldsymbol{\beta}_k$ | 
...
### Additional Parameters to Handle Missing Data ###

The parameter `missing` is logical and indicates if the emission data contains missing observations. In this case, `len.imp` specifies the number of imputations to save post burn-in. The parameter `holdout` is optional and is the same class and dimension of `y` but specifies which observations are in the holdout data set. In `holdout`, 0 denotes observed, 1 denotes MAR, and 2 denotes below LOD. The only reason to specify `holdout` is when evaluating the multiple imputation approach on an incomplete data set: `holdout` specifies the additional data that was removed for evaluation, but not the observations that are missing in the original data set. 

The function also includes parameters that control the independence Metropolis-Hastings (MH) sampler and resolvent kernel used to sample the parameters of $\boldsymbol{\Sigma}_k$ when there is missing data. Details can be found in the Supporting Information of [Hoskovec et al.]. The independence MH sampler uses the following the parameters 
...
We demonstrate each model here. First we fit the homogenous PSPB iHMM to the simulated data. Since our data has missing observations, we specify the tuning parameters for the MH independence sampler. Here we run the model for 200 iterations with 100 burn-in and save 50 imputations post burn-in. On your own data, be sure to run the model for a long enough chain to reach convergence. 

(eval = FALSE)


```{r, eval = FALSE, cache = TRUE}
fit_no_cov = miHMM(niter = 200, nburn = 100, y = dat$y, ycomplete = dat$y.complete, 
...
```{r, echo = FALSE}
fit_no_cov = readRDS("~/Documents/Lauren/Rpackages/psbpHMM/data/fit_no_cov.rds")
```
...
```{r, eval = FALSE, cache = TRUE}
fit_cyclical = mciHMM(niter = 200, nburn = 100, y = dat$y, ycomplete = dat$y.complete, 
...
```{r, echo = FALSE}
fit_cyclical = readRDS("~/Documents/Lauren/Rpackages/psbpHMM/data/fit_cyclical.rds")
```
...
```{r, eval = FALSE, cache = TRUE}
fit_ss = mciHMM(niter = 200, nburn = 100, y = dat$y, rmlist = rmlist, ycomplete = dat$y.complete, 
             X = X, z.true = dat$z.true, lod = dat$lod, mu.true = dat$mu.true, 
             missing = TRUE, tau2 = 0.25, a.tune = 10, b.tune = 1, resK = TRUE, eta.star = 5, 
             len.imp = 50)
```
...
```{r, echo = FALSE}
fit_ss = readRDS("~/Documents/Lauren/Rpackages/psbpHMM/data/fit_ss.rds")
```

# Analyze Results #

The package provides the function `getJointSimResults` to return the evaluation criteria measures used in our simulation study in [Hoskovec et al.]. This function returns the hamming distance (hamming) and MSE for state-specific means (mu.mse) averaged over the iterations post burn-in. It also returns the MSE for MAR data (mar.mse) and data below the LOD (lod.mse) average over the specified number of imputations post burn-in. We can get the summary for each of the models we fit. 
...
To summarize the results from the independently fit models, use the function `getIndepSimResults`. Here, you must also include the data as a parameter. 

```{r, cache = TRUE}
getIndepSimRsults(fit1 = fit_indep_cyclical, y = dat$y)
```

Finally, we include in our package a function to fit a truncated Dirichlet process mixture model (DPMM) jointly to multiple time series. 
