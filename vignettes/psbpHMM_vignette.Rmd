...
We can plot the data to see what it looks like. We plot one exposure for one time series. 
...
```{r, eval = FALSE, cache = TRUE}
fit_cyclical = mciHMM(niter = 1000, nburn = 500, y = dat$y, ycomplete = dat$y.complete, 
...
```{r, eval = FALSE, cache = TRUE}
fit_ss = mciHMM(niter = 1000, nburn = 500, y = dat$y, rmlist = rmlist, ycomplete = dat$y.complete, 
             X = X, z.true = dat$z.true, lod = dat$lod, mu.true = dat$mu.true, 
             missing = TRUE, tau2 = 0.25, a.tune = 10, b.tune = 1, resK = TRUE, eta.star = 5, 
             len.imp = 100)
```
...
fit_indep_cyclical = mclapply(1:20, FUN = function(i){
  mciHMM(niter = 1000, nburn = 500, y = dat$y[[i]], ycomplete = dat$y.complete[[i]], 
...
Finally, we include in our package a function to fit a truncated Dirichlet process mixture model (DPMM) jointly to multiple time series. Here, `K.start` specifies the maximum number of hiddens states allowed. We set `K.start = 50`. 

```{r, eval = FALSE, cache = TRUE}
fit_dpmm = fitDPMM(niter = 1000, nburn = 500, y = dat$y, ycomplete = dat$y.complete, 
             K.start = 50, z.true = dat$z.true, lod = dat$lod, 
             mu.true = dat$mu.true, missing = TRUE, 
             tau2 = 0.25, a.tune = 10, b.tune = 1, resK = TRUE, eta.star = 3, len.imp = 100)
```
...
Now we can compare our simulation results for all of the methods we fit. 
