...
fun2 <- function(t, Xi, beta0, beta.ik){
  t(sapply(1:(K), FUN = function(j){
    first = sapply(1:K, FUN = function(k) pnorm(alpha.jk[[j]][k] + crossprod(Xi[1,],beta0[[k]]) + crossprod(Xi[t,],beta.ik[[k]])))
    second = sapply(1:(K-1), FUN = function(k) 1-pnorm(alpha.jk[[j]][k] + crossprod(Xi[1,],beta0[[k]]) + crossprod(Xi[t,],beta.ik[[k]])))
