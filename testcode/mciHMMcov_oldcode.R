...
# fitMarkovCovariates <- function(niter, nburn, y, ycomplete=NULL, X,
...
################################################################
### this allows X_i to differ for each i ###
### so we have a T-length list of K by K matrices for each i ###
################################################################
...
fun1 <- function(Xi){
  first1 = sapply(1:K, FUN = function(k) pnorm(alpha.0k[k] + crossprod(Xi[1,],beta.k[[k]])))
  second1 = sapply(1:(K-1), FUN = function(k) 1-pnorm(alpha.0k[k] + crossprod(Xi[1,],beta.k[[k]])))
...
fun2 <- function(t, Xi){
  t(sapply(1:(K), FUN = function(j){
    first = sapply(1:K, FUN = function(k) pnorm(alpha.jk[[j]][k] + crossprod(Xi[t,],beta.k[[k]])))
    second = sapply(1:(K-1), FUN = function(k) 1-pnorm(alpha.jk[[j]][k] + crossprod(Xi[t,],beta.k[[k]])))
...
uppi <- function(t, Xi){
  if(t == 1) return(fun1(Xi))
  else return(fun2(t, Xi))
}

################################
### this piece is super slow ###
################################
