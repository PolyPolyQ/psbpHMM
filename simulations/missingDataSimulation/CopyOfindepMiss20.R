# 20% percent missing simulation INDEP 
# rm(list=ls())
# gc()
...
# ### look at MH ratio -- see if resK needs to be FALSE - looks good 10/2/2020
# MH1 <- numeric()
# MH2 <- numeric()
# for(i in 1:n){
#   MH1 <- c(MH1, fit1[[i]]$MH.arate)
#   MH2 <- c(MH2, fit1[[i]]$MH.lamrate)
# }
...
rownames(missingResults) <- c("full20", "nox20")
colnames(missingResults) <- NULL

#cat results/ind20miss*.csv > combined_results/miss20IndResults.txt

list.save(dat1, file = paste0("/projects/lvheck@colostate.edu/markovPSBP/simulations/results/indmiss20data", simnum, ".rds"))
write.table(missingResults, file = paste0("/projects/lvheck@colostate.edu/markovPSBP/simulations/results/ind20miss", simnum, ".csv"), 
