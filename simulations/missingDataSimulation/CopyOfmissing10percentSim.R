...
# i2 = 2
# 
# plot(1:t.max, dat1$y.complete[[i1]][,1], col = dat1$z.true[[i1]], pch = 19)
# abline(h = dat1$lod[1], col = "black", lwd = 2)
# plot(1:t.max, dat1$y.complete[[i1]][,2], col = dat1$z.true[[i1]], pch = 19)
# abline(h = dat1$lod[2], col = "black", lwd = 2)
# plot(1:t.max, dat1$y.complete[[i1]][,3], col = dat1$z.true[[i1]], pch = 19)
# abline(h = dat1$lod[3], col = "black", lwd = 2)
# plot(1:t.max, dat1$y.complete[[i2]][,1], col = dat1$z.true[[i2]], pch = 19)
# abline(h = dat1$lod[1], col = "black", lwd = 2)
# plot(1:t.max, dat1$y.complete[[i2]][,2], col = dat1$z.true[[i2]], pch = 19)
# abline(h = dat1$lod[2], col = "black", lwd = 2)
# plot(1:t.max, dat1$y.complete[[i2]][,3], col = dat1$z.true[[i2]], pch = 19)
# abline(h = dat1$lod[3], col = "black", lwd = 2)


# # -Inf < below LOD in complete data because some of those might be NA
# length(which(unlist(dat1$y)==-Inf))
# ycomp <- rbind(dat1$y.complete[[1]], dat1$y.complete[[2]])
# length(which(ycomp[,1]<dat1$lod[1]))
# length(which(ycomp[,2]<dat1$lod[2]))
# length(which(ycomp[,3]<dat1$lod[3]))
...
# check mixing 
fit1cyc$MH.arate; fit1cyc$MH.lamrate
fit1base$MH.arate; fit1base$MH.lamrate
fitdpmm$MH.arate; fitdpmm$MH.lamrate
...
rownames(missingResults) <- c("full10", "nox10", "dpmm10")
colnames(missingResults) <- NULL

mr <- missingResults; mr

#cat results/miss10percent*.csv > combined_results/miss10percentResults.txt

list.save(dat1, file = paste0("/projects/lvheck@colostate.edu/markovPSBP/simulations/results/miss10data", simnum, ".rds"))
write.table(missingResults, file = paste0("/projects/lvheck@colostate.edu/markovPSBP/simulations/results/miss10percent", simnum, ".csv"), 
